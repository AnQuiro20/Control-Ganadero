document.addEventListener('DOMContentLoaded', function () {
    ...
    animalForm.addEventListener('submit', async function (e) {
        ...
        document.getElementById('earTag').addEventListener('blur', function () {
            validateEarTag(this.value);
        });

        function validateEarTag(earTag) {
            const input = document.getElementById('earTag');
            if (!earTag) return;

            if (!isEarTagUnique(earTag)) {
                input.classList.add('is-invalid-earTag');
                Swal.fire({
                    title: 'Arete duplicado',
                    text: 'Este número de arete ya está registrado',
                    icon: 'warning',
                    timer: 2000
                });
            } else {
                input.classList.remove('is-invalid-earTag');
            }
        }
    ...
    document.getElementById('exportExcel').onclick = function () {
        ...
        const data = [
            ['Arete', 'Tipo', 'Nombre', 'Edad', 'Fecha Nacimiento', 'Raza', 'Sexo', 'Partos', 'Fecha Registro', 'Observaciones'],
